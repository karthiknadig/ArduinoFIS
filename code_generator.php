...
function GetCodeBase()
{
    ...
    $code_base['zmf'] = file_get_contents("codebase/fis_zmf.c");
    return $code_base;
}
...
{
    return str_replace("/*FIS_Version*/", "2.0.1.25122016", $content);
}
...
function SetFISMFOutputsCoeffs($fis, $content)
{
    ...
    for($i=0;$i<$size;$i++)
    {
        ...
        for($j=1;$j<=$mfc;$j++)
        {
            ...
            $constant = strtolower(trim($fis["Output"][$i]["MF$j"]['MF']));
            if(strcasecmp($constant, "constant")==0)
            {
                $inputSize = intval($fis['System']['NumInputs']);
...
function SetFISOutputMFs($fis, $mfs, $content)
{
    ...
    {
        $content = str_replace("/*FIS_OutputMFs*/", "", $content);
        return str_replace("/*FIS_OutputMFsList*/", "", $content);
    }
}
...
function SetFISFuzzyAndOrInits($fis, $content)
{
    // Initial values for And operation types
    $andinits = array();
    $andinits['min'] = "FIS_MAX"; // Init mins with the largest value
    $andinits['prod'] = "1";

    $orinits = array();
    $orinits['max'] = "FIS_MIN"; // Init max with the smallest value
    $orinits['probor'] = "0";

    $content = str_replace("/*FIS_AndInit*/", $andinits[trim($fis['System']['AndMethod'])], $content);
    $content = str_replace("/*FIS_OrInit*/", $orinits[trim($fis['System']['OrMethod'])], $content);
    return $content;
}
...
function GetMainFileContent($fis)
{
    ...
    $content = SetFISFuzzyAndOrInits($fis, $content);
